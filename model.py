...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        print("[D4] SEBlock Channels:", scale.flatten()[:20])
        print("  FC1 Weights:", self.fc1.weight.data.view(-1)[:5])
        print("  FC2 Weights:", self.fc2.weight.data.view(-1)[:5])
        print("  Scale Values:", scale.detach().flatten()[:10]) 
        return x * scale
